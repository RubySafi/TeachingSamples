...
type StepType =
  ...
  | 'diff'
  | 'carry'
  | 'decimal_mark' // 小数点を打つ（確認ステップ）
  | 'decimal_carry' // 被除数の 0 を降ろす（入力ステップ）
  | 'conclusion';
...
function generateSteps(
  ...
  const iStart = quotientSeq.findIndex(
    (q, idx) => idx < integerDigits && q !== 0
  );
...
function makeHelpers(ctx: CanvasRenderingContext2D, cfg: DrawConfig) {
  ...
  function putTextAtDigit(
    val: number,
    row: number,
    rightDigitIdx: number,
    color?: string
  ) {
    const s = String(val);
  ...
  function putDividendDecimalExt(
    numZeros: number,
    row: number,
    color?: string
  ) {
  ...
  /** マスク矩形（入力待ちセル） */
  function drawMask(row: number, rightDigitIdx: number, digits: number) {
    const rightCol = digitToCol(rightDigitIdx);
    const leftCol = rightCol - digits + 1;
  ...
  function drawQuotientMask(digitIdx: number) {
  ...
  if (decimalShown) {
    ...
    for (let s = 1; s <= upToStep; s++) {
      ...
      if (st.type === 'decimal_carry') {
        ...
        st.digitIndex !== undefined &&
        st.digitIndex >= integerDigits
      ) {
        maxDecimalIdx = Math.max(
          maxDecimalIdx,
          st.digitIndex - integerDigits + 1
        );
      }
    }
...
function initUI(dividend: number, divisor: number) {
  ...
  const canvas = document.getElementById('s2-canvas') as HTMLCanvasElement;
  ...
  const inputEl = document.getElementById('s2-input') as HTMLInputElement;
  ...
  const btnCheck = document.getElementById('s2-btn-check') as HTMLButtonElement;
  ...
  const newBtnCheck = btnCheck.cloneNode(true) as HTMLButtonElement;
  ...
  function flash(correct: boolean, onDone?: () => void) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const btnNew = document.getElementById('s2-btn-new') as HTMLButtonElement;
  ...
  btnNew.addEventListener('keydown', (e: KeyboardEvent) => {
