...
interface ProblemPattern {
  ...
  generate: (units: string[], nums: number[]) => GeneratedProblem;
}
...
/** --- ユーティリティ --- **/
const getRandomUnits = (n: number): string[] => {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const choiceBtns = [0, 1, 2, 3].map(
    (i) => document.getElementById(`sec03-choice-${i}`) as HTMLButtonElement
  );

  const lvBtns: { [key: number]: HTMLButtonElement } = {
    1: document.getElementById('sec03-lv-1') as HTMLButtonElement,
    2: document.getElementById('sec03-lv-2') as HTMLButtonElement,
    3: document.getElementById('sec03-lv-3') as HTMLButtonElement,
  };
  ...
  const handleChoiceClick = (btn: HTMLButtonElement) => {
