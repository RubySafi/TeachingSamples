...
const leapYearCheckbox5 = document.getElementById(
  'sec5-leapyear-checkbox'
) as HTMLInputElement;

const speedSlider5 = document.getElementById(
  'sec5-speed-slider'
) as HTMLInputElement;
const speedValue5 = document.getElementById('sec5-speed-value')!;
const spdList5 = [-50, -5, -1, 0, 1, 5, 50];

const formatSpeed5 = (n: number) =>
  `${n < 0 ? '&minus;' + Math.abs(n) : n} h/s`;
...
function isLeapYear5(year: number): boolean {
  return leapYearCheckbox5.checked;
}

function getDaysInMonth5(year: number, month: number): number {
  if (month === 2) {
...
function formatDateTime5(date: Date): string {
