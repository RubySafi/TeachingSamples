...
let currentAngle = 0;
let currentShape: { x: number; y: number }[] = [];

// DOM要素の取得
const minoContainer = document.getElementById(
  'sec02-mino-container'
) as HTMLElement;
const angleDisplay = document.getElementById('sec02-angle-info') as HTMLElement;

// 2. ミノを生成して画面に配置する関数
function displayShape(shape: { x: number; y: number }[]) {
...
// 2. ミノを生成して画面に配置する関数
function displayShapeDebug(shape: { x: number; y: number }[]) {
  // 一旦中身を空にする (WPFの Children.Clear() )
  minoContainer.innerHTML = '';
