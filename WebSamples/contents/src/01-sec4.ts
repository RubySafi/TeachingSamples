...
const sec4_leapYearCheckbox = document.getElementById(
  'sec4-leapyear-checkbox'
) as HTMLInputElement;
...
function sec4_isLeapYear(): boolean {
  return sec4_leapYearCheckbox.checked;
}

function sec4_getBaseYear(): number {
  return sec4_isLeapYear() ? 2024 : 2023;
}

function sec4_getDaysInMonth(year: number, month: number): number {
  if (month === 2) {
...
function sec4_formatDate(date: Date): string {
...
function sec4_addMonths(months: number) {
  sec4_fixBaseYear();
  ...
  const daysInMonth = sec4_getDaysInMonth(
    sec4_getBaseYear(),
    sec4_currentDate.getMonth() + 1
  );
...
function sec4_addDays(days: number) {
  sec4_fixBaseYear();
