...
let unitsInOrder: string[] = [];

const processDisplay = document.getElementById(
  '02-unit-process'
) as HTMLDivElement;
...
const formatExp = (num: number): string => {
...
const updateDisplay = () => {
  ...
  if (history.length === 0) {
    ...
    processDisplay.innerHTML = history
      .map((item, index) => {
        ...
        const symbol =
          index === 0 ? '' : item.op === 'mul' ? ' &times; ' : ' &divide; ';
  ...
  history.forEach((item) => {
    if (!counts[item.unit]) counts[item.unit] = 0;
...
const addUnit = (type: 'mul' | 'div', unit: string) => {
