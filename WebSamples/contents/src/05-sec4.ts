...
const gridModel_X: number = 4;
const gridModel_Y: number = 8;
const gridMain_X: number = 8;
const gridMain_Y: number = 8;

let ans: { x: number; y: number };
let shapeSize: { x: number; y: number };
let shapeModelInfo: { x: number; y: number; redflag: boolean }[];
let shapeMainState = Array.from({ length: gridMain_X }, () =>
  Array(gridMain_Y).fill(false)
); //MainCanvasの現在の状況
let shapeAnswerState = Array.from({ length: gridMain_X }, () =>
  Array(gridMain_Y).fill(false)
); //MainCanvasの解答
let redTile: { x: number; y: number }; //MainCanvasの赤タイルの位置

const infoElement = document.getElementById(
  'sec4_slideGoalInfo'
) as HTMLElement;

const btnReset = document.getElementById('sec4_reset') as HTMLButtonElement;
const btnQuiz = document.getElementById('sec4_quiz') as HTMLButtonElement;
const btnQuizHard = document.getElementById('sec4_quiz2') as HTMLButtonElement;
...
function createNewHard() {
  const tempShape: { x: number; y: number }[] = [];
...
function createNew() {
  ...
  const pattern =
    SHAPE_PATTERNS[Math.floor(Math.random() * SHAPE_PATTERNS.length)];
...
function InitializeAnswerState() {
  //初期化
  shapeAnswerState = Array.from({ length: gridMain_X }, () =>
    Array(gridMain_Y).fill(false)
  );
...
function setBlock(e: MouseEvent) {
  ...
  if (x >= 0 && x < gridMain_X && y >= 0 && y < gridMain_Y) {
    // 赤タイルは消せないようにガード
    if (x === redTile.x && y === redTile.y) return;
...
function eraseBlock(e: MouseEvent) {
  e.preventDefault();
  const rect = mainCanvas.getBoundingClientRect();
  const x = Math.floor((e.clientX - rect.left) / gridSize);
  const y = Math.floor((e.clientY - rect.top) / gridSize);
...
function reset(): void {
  ...
  shapeMainState = Array.from({ length: gridMain_X }, () =>
    Array(gridMain_Y).fill(false)
  );
