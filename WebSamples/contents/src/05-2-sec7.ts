...
const QUIZ_PATTERNS_LOW: RelativePattern[] = [
  {
    name: 'domino',
    coords: [
      { x: 0, y: 0 },
...
const QUIZ_PATTERNS_TETROMINO: RelativePattern[] = [
  {
    name: 'I-Tetro',
    coords: [
      { x: 0, y: 0 },
...
const QUIZ_PATTERNS_PENTOMINO: RelativePattern[] = [
  {
    name: 'F-Pento',
    coords: [
      { x: 0, y: 0 },
...
let currentOriginalCoords: { x: number; y: number }[] = [];
let currentTargetAngle: number;
let currentOffset = { x: 0, y: 0 };
let currentPivotIndex: number = 0;
...
function rotateCoords(coords: { x: number; y: number }[], degree: number) {
  return coords.map((p) => {
    ...
    if (degree === 180 || degree === -180) return { x: -p.x, y: -p.y };
    return { x: p.x, y: p.y };
  });
}
 ...
 * 座標反転関数
 */
function flipCoords(coords: { x: number; y: number }[]) {
 ...
 * クイズ生成
 */
function generateQuiz7(quizPatterns: RelativePattern[]) {
  ...
  if (Math.random() > 0.5) tempCoords = flipCoords(tempCoords);
 ...
 */
function runHint7() {
  if (isHintRunning || currentOriginalCoords.length === 0) return;
  isHintRunning = true;
  ...
  currentOriginalCoords.forEach((p, i) => {
    ...
    if (i === currentPivotIndex) div.style.borderRadius = '4px';
...
function createTileElement(x: number, y: number, offset: number) {
