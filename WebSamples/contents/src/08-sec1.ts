...
type StepType =
  ...
  | 'frame'       // Step 1: bracket, divisor, dividend
  | 'look'        // highlight first usable digit group (when tens digit < divisor)
  | 'quotient'    // write quotient digit
  | 'product'     // write divisor × quotient digit
  | 'line'        // draw subtraction line
  | 'diff'        // write the remainder
  | 'carry'       // bring down next digit
  | 'decimal'     // place the decimal point (quotient + dividend extension)
...
interface Step {
  ...
  label: string;
  caption: string;
  /** digit index into the extended sequence (0 = tens of dividend, 1 = ones, 2 = first decimal place, …) */
  digitIndex?: number;
}
...
/**
 * 2桁÷1桁の小数割り算のシーケンスを生成する。
 * 整数部(2桁)＋小数部(必要な桁数)を含む「拡張された被除数桁列」を作る。
 * 例: 13 ÷ 4 → digits = [1, 3, 0, 0] (13.00), result 3.25
 */
interface DecimalSequence {
  /** 被除数の桁列（整数部2桁 ＋ 小数部の0たち） */
  dividendDigits: number[];
  /** totalSeq[i] = そのステップで割り算する数 */
  totalSeq: number[];
  /** quotientSeq[i] = そのステップの商の桁 */
  quotientSeq: number[];
  /** divisionSeq[i] = divisor × quotientSeq[i] */
  divisionSeq: number[];
  /** subtractionSeq[i] = totalSeq[i] - divisionSeq[i] */
...
function generateDecimalSequence(dividend: number, divisor: number): DecimalSequence {
  ...
  const dividendDigits: number[] = intStr.split('').map(Number);
  for (let k = 0; k < decimalDigits; k++) dividendDigits.push(0);
...
function generateSteps(
  ...
  const { totalSeq, quotientSeq, divisionSeq, subtractionSeq,
          integerDigits, decimalDigits, dividendDigits } = seq;
  ...
  for (let i = (iStart >= 0 ? iStart : 0); i < totalLen; i++) {
    ...
    if (q === 0) {
      if (isLast) break; // 最終桁で q=0 は起こらないが念のため
...
function buildQuotientString(quotientSeq: number[], integerDigits: number): string {
...
function makeHelpers(ctx: CanvasRenderingContext2D, cfg: DrawConfig) {
   ...
   */
  function digitToCol(i: number): number {
    return 1 + i;
  }
   ...
   * 複数桁の場合は左に広がる。
   */
  function putTextAtDigit(val: number, row: number, rightDigitIdx: number, color?: string) {
    const s = String(val);
  ...
  function putQuotientDigit(val: number, digitIdx: number, color?: string) {
    setFont(28, color ?? '#2c2416');
    ctx.fillText(String(val), cenX(digitToCol(digitIdx)), cenY(-1));
  }

  /** 商の小数点：グリッド交差点（整数部右端の境界線上）に描画 */
  function putQuotientDecimalPoint(color?: string) {
  ...
  /** 被除数（整数部のみ）を row に描画 */
  function putDividendInt(dividend: number, row: number, color?: string) {
  ...
  /** 被除数の小数部「.0…」を描画（拡張表示用） */
  function putDividendDecimalExt(numZeros: number, row: number, color?: string) {
    ctx.font = '28px "JetBrains Mono", monospace';
   ...
   * 横線は整数部＋小数部すべてをカバー
   */
  function drawFrame(showDecimalExt: boolean) {
  ...
  function drawResultLine(row: number, rightDigitIdx: number, digits: number) {
...
function render(
  ...
  const { integerDigits, decimalDigits, totalSeq, quotientSeq,
          divisionSeq, subtractionSeq, dividendDigits } = seq;
  ...
  for (let s = 1; s <= upToStep; s++) {
    if (steps[s]?.type === 'decimal') { decimalShown = true; break; }
  }
  ...
  for (let s = 2; s <= upToStep; s++) {
    ...
    if (i === undefined && step.type !== 'conclusion') continue;
