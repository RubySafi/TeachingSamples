...
//
const bPracticeZero: boolean = true;
...
/**
 * requiresInput: true のステップでは入力フィールドを表示し，
 * Check ボタンで expectedAnswer と照合する．
 *
 * carry の expectedAnswer は totalSeq[i+1] ではなく，
 * 降ろしてくる「1 桁」= parseInt(dividendStr[i+1]) とする．
 */
interface Step {
  type: StepType;
  label: string;
  instruction: string; // 入力欄の上に表示する短い指示
  digitIndex?: number;
...
function makeHelpers(ctx: CanvasRenderingContext2D, cfg: DrawConfig) {
   ...
   */
  function drawMask(row: number, place: number, digits: number) {
    const rightCol = placeToCol(place);
   ...
   */
  function drawQuotientMask(place: number) {
    drawMask(-1, place, 1);
  }
    ...
    switch (step.type) {
      ...
      case 'diff': {
        if (isCurrentStep && step.requiresInput) {
          ...
          h.putMainTextAtPlace(
            seq.subtractionSeq[i],
            rowBase + 2,
            place,
            color
          );
