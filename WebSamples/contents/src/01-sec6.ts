...
const sec6_leapYearCheckbox = document.getElementById(
  'sec6-leapyear-checkbox'
) as HTMLInputElement;
...
function sec6_isLeapYear(): boolean {
  return sec6_leapYearCheckbox.checked;
}

function sec6_getBaseYear(): number {
  return sec6_isLeapYear() ? 2024 : 2023;
}

function sec6_getDaysInMonth(year: number, month: number): number {
  if (month === 2) {
...
function sec6_formatDateTime(date: Date): string {
...
function sec6_addHours(hours: number) {
  sec6_fixBaseYear();
  ...
  // 年のズレや存在しない日補正
  sec6_fixBaseYear();
  const daysInMonth = sec6_getDaysInMonth(
    sec6_getBaseYear(),
    sec6_currentDate.getMonth() + 1
  );
...
function sec6_addMinutes(minutes: number) {
  sec6_fixBaseYear();
