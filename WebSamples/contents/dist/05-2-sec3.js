...
// --- 座標回転ロジック ---
function rotateCoords(shape, degree, flip = false) {
    return shape.map((p) => {
        let nx = flip ? -p.x : p.x;
        let ny = p.y;
        if (degree === 90)
            return { x: -ny, y: nx }; // CW 90
        if (degree === 180)
            return { x: -nx, y: -ny }; // 180
        if (degree === 270 || degree === -90)
            return { x: ny, y: -nx }; // CCW 90
        return { x: nx, y: ny }; // 0
    });
}
// --- クイズ生成 ---
...
// --- 判定ロジック ---
function checkAnswer(isCorrect, directionName) {
    const questionText = document.getElementById('sec03-question-text');
    if (isCorrect) {
...
// --- 描画ヘルパー ---
function drawShape(containerId, shape, size) {
