...
const leapYearCheckbox = document.getElementById('sec3-leapyear-checkbox');
const speedSlider = document.getElementById('sec3-speed-slider');
const speedValue = document.getElementById('sec3-speed-value');
const spdList = [-5, -1, 0, 1, 5];
const formatSpeed = (n) => `${n < 0 ? '&minus;' + Math.abs(n) : n} day/s`;
...
function animate(now) {
    ...
    currentDate.setTime(currentDate.getTime() + dayProgress * 24 * 60 * 60 * 1000);
    currentDate.setFullYear(baseYear);
    updateDateLabels();
    updateCalendarTable();
