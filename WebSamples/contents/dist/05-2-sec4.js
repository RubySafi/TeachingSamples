...
let currentRelativePattern = null;
/**
 * 矢印要素を生成する関数
 */
function createArrow(angleDeg, color = '#2c3e50') {
 ...
 * ミノを描画する
 */
function renderMinos(container, coords, isGhost) {
    if (!currentRelativePattern)
        return;
...
function drawRelative(containerId, targetAngle) {
    const container = document.getElementById(containerId);
    if (!container || !currentRelativePattern)
        return;
    container.innerHTML = '';
    // 1. 静止している「正解の状態」を描画
